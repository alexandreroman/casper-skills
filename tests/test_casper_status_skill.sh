...
grep -q "^allowed-tools:.*casper status set" "$FILE" || fail "missing allowed-tools pre-authorization for casper status set"
grep -q "^allowed-tools:.*casper notify" "$FILE" || fail "missing allowed-tools pre-authorization for casper notify"
